...
BEGIN
  ...
  EXECUTE 'INSERT INTO ' || dest_schema || '.permissions (select * from ' || source_schema || '.permissions)';
  EXECUTE 'INSERT INTO ' || dest_schema || '.roles (select * from ' || source_schema || '.roles)';
  EXECUTE 'INSERT INTO ' || dest_schema || '.roles_permissions (select * from ' || source_schema || '.roles_permissions)';
